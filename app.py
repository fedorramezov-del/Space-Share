...
ALLOWED_EXTENSIONS = {
    ...
    # ДОКИ
    'pdf', 'docx', 'doc', 'odt', 'txt', 'md', 'csv', 'json',"pptx",
    
    # КОДЫ ЛОГИ И ВСЯКАЯ ФИГНЯ
    'html', 'css', 'js', 'py', 'xml', 'ini', 'cfg', 'log','jar',
...
def create_app(gui_signal=None):
    ...
    def download_batch(batch_id):
        ...
        zip_name = f"batch_{batch_id[:8]}.zip"
        zip_path = os.path.join(temp_dir, zip_name)
        
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
            for f in files:
                file_path = os.path.join(app.config['UPLOAD_FOLDER'], f.storage_filename)
    ...
    @app.route('/preview_archive/<int:file_id>')
    @login_required
    def preview_archive(file_id):
        ...
        ext = file_record.filename.lower().split('.')[-1]
        if ext not in ['zip', 'rar', '7z', 'tar', 'gz']:
            flash('Это не архив', 'error')
                ...
                with zipfile.ZipFile(file_path, 'r') as zf:
                    for info in zf.infolist():
                        contents.append({
                            ...
                            'is_dir': info.is_dir(),
                            'modified': datetime(*info.date_time).strftime('%d.%m.%Y %H:%M')
                        })
            elif ext in ['rar']:
                try:
                    import rarfile
                    with rarfile.RarFile(file_path, 'r') as rf:
                        for info in rf.infolist():
                            contents.append({
                                ...
                                'compressed': info.compress_size,
                                'is_dir': info.is_dir(),
                                'modified': datetime(*info.date_time).strftime('%d.%m.%Y %H:%M') if info.date_time else 'Нет данных'
                            })
                except ImportError:
                    contents = [{'name': 'rarfile не установлен (pip install rarfile)', 'size': 0, 'is_dir': False}]
            elif ext in ['7z']:
                try:
                    import py7zr
                    with py7zr.SevenZipFile(file_path, 'r') as szf:
                        for info in szf.list():
                            contents.append({
                                ...
                                'compressed': info.compressed if hasattr(info, 'compressed') else 0,
                                'is_dir': info.is_directory if hasattr(info, 'is_directory') else False,
                                'modified': 'Нет данных'
                            })
                except ImportError:
                    contents = [{'name': 'py7zr не установлен (pip install py7zr)', 'size': 0, 'is_dir': False}]
            elif ext in ['tar', 'gz']:
                import tarfile
                with tarfile.open(file_path, 'r') as tf:
                    for info in tf.getmembers():
                        contents.append({
                            ...
                            'size': info.size,
                            'is_dir': info.isdir(),
                            'modified': datetime.fromtimestamp(info.mtime).strftime('%d.%m.%Y %H:%M')
                        })
        except Exception as e:
            contents = [{'name': f'Ошибка чтения: {str(e)}', 'size': 0, 'is_dir': False}]
    ...
    def admin_delete_user(user_id):
        from models import Texture, Material, CustomTheme, ChatMessage
        ...
        try:
            # 1. Удаляем файлы пользователя и связанные с ними текстуры/материалы
            files = File.query.filter_by(user_id=user.id).all()
            for file in files:
                try:
                    # Удаляем текстуры для этой модели
                    ...
                    materials = Material.query.filter_by(model_file_id=file.id).all()
                    for material in materials:
                        site.session.delete(material)
                        deleted_materials += 1
                    ...
                    app.logger.error(f"Ошибка удаления файла {file.id}: {e}")
                    errors += 1
            ...
            if user.avatar:
                ...
                if os.path.exists(avatar_path):
                    try:
                        ...
                        app.logger.error(f"Ошибка удаления аватара: {e}")
            
         
            themes = CustomTheme.query.filter_by(user_id=user.id).all()
            for theme in themes:
                site.session.delete(theme)
                deleted_themes += 1
            

            messages = ChatMessage.query.filter_by(user_id=user.id).all()
            for msg in messages:
            ...
            result_msg = f"Пользователь {user.username} удалён. "
            result_msg += f"Файлов: {deleted_files}, текстур: {deleted_textures}, "
            result_msg += f"материалов: {deleted_materials}, тем: {deleted_themes}, "
            result_msg += f"сообщений: {deleted_messages}"
            
            if errors > 0:
                result_msg += f". Ошибок: {errors}"
            ...
            site.session.rollback()
            app.logger.error(f"Ошибка удаления пользователя {user_id}: {e}")
    ...
    def api_online_users():
        ...
        # Считаем онлайн тех, кто был активен за последние 5 минут
        ...
        for user in online_users_query:
            # Получаем часовой пояс пользователя (или админа)
            tz_str = getattr(current_user, 'timezone', 'Europe/Moscow')
            try:
            ...
            utc_time = user.last_seen.replace(tzinfo=timezone.utc)
            local_time = utc_time.astimezone(user_tz)
            
            online_users.append({
                'id': user.id,
                'username': user.username,
                'avatar': url_for('avatar', filename=user.avatar) if user.avatar else url_for('static', filename='default_avatar.png'),
                'role': user.role,
                'last_seen': local_time.strftime('%H:%M:%S'),
                'last_seen_full': local_time.strftime('%d.%m.%Y %H:%M:%S'),
                'in_chat': user.id in online_users_set
            })
        ...
        return jsonify({
            ...
            'server_timezone': str(current_user.timezone) if current_user.is_authenticated else 'Europe/Moscow'
        })
    ...
    voice_rooms = {}  # { roomId: { name, password, participants: { sid: {...} } } }
    ...
    def handle_voice_join(data):
        ...
        if room_id == 'general' and room_id not in voice_rooms:
            voice_rooms[room_id] = {
                'name': 'General',
                'password': None,
                'participants': {}
        ...
        if room_id not in voice_rooms:
            socketio.emit('voice:error', {'message': 'Room not found'}, room=request.sid)
            return
        
        room = voice_rooms[room_id]
        
        if room.get('password') and room['password'] != password:
            socketio.emit('voice:error', {'message': 'Wrong password'}, room=request.sid)
            return
        ...
        socketio.emit('voice:participants', {
            'participants': participants_list,
            'roomName': room['name']
        }, room=room_id)

    # Получение участников комнаты БЕЗ подключения
    @socketio.on('voice:peek')
    def handle_voice_peek(data):
        room_id = data.get('roomId', 'general')
        
        if room_id not in voice_rooms:
            socketio.emit('voice:peek-result', {'participants': [], 'roomName': room_id}, room=request.sid)
            return
        ...
        socketio.emit('voice:peek-result', {
            'participants': participants_list,
            'roomName': room['name']
        }, room=request.sid)
    ...
    def handle_voice_leave(data=None):
        room_id = data.get('roomId', 'general') if data else 'general'
        
        if room_id in voice_rooms:
            room = voice_rooms[room_id]
            if request.sid in room['participants']:
                room['participants'].pop(request.sid)
                ...
                if len(room['participants']) == 0 and room_id != 'general':
                    del voice_rooms[room_id]
        
        leave_room(room_id)

    @socketio.on('voice:create-room')
    def handle_create_room(data):
        ...
        socketio.emit('voice:room-created', {
            'id': room_id,
            'name': room_name,
            'hasPassword': bool(password)
        })

    @socketio.on('voice:toggle-mute')
    def handle_voice_toggle_mute(data):
        """Пользователь переключает микрофон"""
        room_id = data.get('roomId', 'general')
        ...
        if room_id in voice_rooms:
            room = voice_rooms[room_id]
            if request.sid in room['participants']:
                room['participants'][request.sid]['muted'] = is_muted
                socketio.emit('voice:user-muted', {
                    'sid': request.sid,
                    'muted': is_muted
                }, room=room_id)
    ...
    def handle_voice_speaking(data):
        ...
        if room_id in voice_rooms:
            room = voice_rooms[room_id]
            if request.sid in room['participants']:
                room['participants'][request.sid]['speaking'] = is_speaking
                socketio.emit('voice:user-speaking', {
                    'sid': request.sid,
    ...
    def handle_webrtc_offer(data):
        """Пересылаем WebRTC offer конкретному участнику"""
    ...
    def handle_webrtc_answer(data):
        """Пересылаем WebRTC answer конкретному участнику"""
        target_sid = data['targetSid']
    ...
    def handle_disconnect():
        """Очистка при отключении пользователя"""
        app.logger.info(f"Client disconnected: {request.sid}")
        
        # Очистка голосовых комнат
        for room_id in list(voice_rooms.keys()):
            room = voice_rooms.get(room_id)
            if room and request.sid in room['participants']:
                user_info = room['participants'].pop(request.sid)
                app.logger.info(f"Voice: removed {user_info.get('username', 'unknown')} from room {room_id}")
                ...
                socketio.emit('voice:participants', {
                    ...
                    'roomName': room.get('name', room_id)
                }, room=room_id)
    ...
    @login_required
    def api_users():
        users = User.query.with_entities(User.id, User.username, User.avatar).all()
        return jsonify({
            'users': [{
                'id': u.id,
                'username': u.username,
 ...
 #авто отчистка чанков
   ...
   #УРААААаАААа победа , инициируем все это дермо
    ...
    ping_timeout=30,       # было 60
    ping_interval=10,      # было 25
