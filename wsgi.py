...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QPushButton, 
                             ...
                             QListWidgetItem, QTabWidget, QStyle, QLineEdit)
...
class ServerLogger:
    ...
    def log(self, level, message):
        ...
        if level_lower in self.loggers:
            log_methods = {
                'INFO': 'info', 'WARNING': 'warning', 
                'ERROR': 'error', 'DEBUG': 'debug'
            }
            method_name = log_methods.get(level, 'info')
            getattr(self.loggers.get(level_lower, self.loggers.get('app')), method_name, lambda x: None)(message)
    ...
    def voice(self, msg):
        """Голосовой чат теперь работает через Socket.IO — просто логируем"""
        self.log('INFO', f"[VOICE] {msg}")
                ...
                while self.running:
                    try:
                        ...
                        if msg == DISCOVERY_MSG:
                            ...
                            server_id = f"{addr[0]}:{DEFAULT_PORT}"
                            if server_id != f"{self.local_ip}:{DEFAULT_PORT}":
                                server_data = {
                                    'id': server_id,
                                    'name': f"Server at {addr[0]}",
                                    'ip': addr[0],
                                    'port': DEFAULT_PORT,
                                    'address': f"https://{addr[0]}:{DEFAULT_PORT}",
                                    'type': 'broadcast'
                                }
...
class FileManager:
    ...
    def create_sample_files(self):
        ...
        templates = {
            ...
            'lan_chat.html': self._get_chat_template(),
            'voice_chat.html': self._get_voice_template(),
            'errors/404.html': self._get_404_template(),
            'errors/403.html': self._get_403_template(),
            'errors/500.html': self._get_500_template()
        }
...
{% endblock %}"""
    ...
    def _get_voice_template(self):
        return """{% extends "base.html" %}
{% block title %}Голосовой чат{% endblock %}
{% block content %}
<h1>Голосовой чат</h1>
<p>WebRTC голосовой чат через Socket.IO</p>
<p>Подключитесь к каналу и общайтесь с другими пользователями</p>
<a href="/voice" class="btn">Перейти в голосовой чат</a>
{% endblock %}"""
...
class ServerThread(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.log_signal.emit(f"[СЕРВЕР] Локальный доступ: https://127.0.0.1:{self._actual_port}")
            self.log_signal.emit(f"[СЕРВЕР] Сетевой доступ: https://{local_ip}:{self._actual_port}")
    ...
    def _start_flask(self):
        ...
        if not os.path.exists(cert_path):
            self.log_signal.emit(f"[ОШИБКА] Сертификат не найден: {cert_path}")
            return
        if not os.path.exists(key_path):
            self.log_signal.emit(f"[ОШИБКА] Ключ не найден: {key_path}")
            return
        
        self.log_signal.emit(f"[СЕРВЕР] Загрузка сертификатов из {BASE_DIR}")
        ...
        try:
            ...
            self.log_signal.emit(f"[ОШИБКА] Не удалось запустить HTTPS: {e}")
            self.log_signal.emit("[ОШИБКА] Проверьте сертификаты")
            raise
...
class AdminWindow(QMainWindow):
    """Главное окно управления сервером"""
    ...
    def __init__(self):
        ...
        self.logger.voice("Голосовой чат работает через Socket.IO + WebRTC")
    ...
    def _setup_server_tab(self):
        ...
        voice_info_frame = QFrame()
        voice_info_frame.setStyleSheet("""
            QFrame {
                background-color: #1a1a1a;
                border: 1px solid #333;
                border-radius: 12px;
        ...
        """)
        voice_info_layout = QHBoxLayout(voice_info_frame)
        
        voice_info_label = QLabel("🎙️ Голосовой чат: WebRTC через Socket.IO (встроен в основной сервер)")
        voice_info_label.setFont(QFont("Segoe UI", 10))
        voice_info_label.setStyleSheet("color: #22c55e;")
        voice_info_layout.addWidget(voice_info_label)
        
        layout.addWidget(voice_info_frame)
    ...
    def on_server_found(self, server_data):
        ...
        if server_id in self.discovered_servers:
            self.discovered_servers[server_id].update(server_data)
            self.logger.discovery(f"Обновлен сервер: {server_data['address']}")
        else:
    ...
    def refresh_servers(self):
        ...
        self.discovery_status.setText("🔍 Поиск серверов...")
        
        if self.discovery_thread and self.discovery_thread.isRunning():
            old_thread = self.discovery_thread
            old_thread.stop()
            old_thread.wait()
    ...
    def check_initial_setup(self):
        ...
        self.append_log("🎙️ Голосовой чат встроен в основной сервер (WebRTC + Socket.IO)")
    ...
    def start_server(self):
        ...
        if not self.server_thread or not self.server_thread.isRunning():
            ...
            self.append_log("🎙️ Голосовой чат будет доступен после запуска (WebRTC + Socket.IO)")
    ...
    def open_browser(self):
        ...
        if self.server_thread:
            ...
            webbrowser.open(f"https://{ip}:{self.config.port}")
            self.logger.info(f"Открыт браузер: https://{ip}:{self.config.port}")
